...
echo "[4] POST translate en->de" && curl -fsS -H 'Content-Type: application/json' -d '{"text":"Hello","targetLang":"DE"}' "$BASE/api/translate" | jq -r .translated | grep -qi 'hallo' && echo " OK"
